/*
 * png_writer.h
 * Minimal PNG encoder — no external libraries required.
 * Writes uncompressed PNG (valid, universally supported).
 *
 * Usage:
 *   #include "png_writer.h"
 *   write_png("output.png", width, height, rgb_buffer);
 */
...
{
    if (!png__crc_ready) png__make_crc_table();
                              ...
                              const unsigned char *data, uint32_t len)
{
    ...
    fwrite(type, 1, 4, fp);
    if (data && len) fwrite(data, 1, len, fp);
                     ...
                     const unsigned char *rgb)
{
    ...
    if (!deflate_data) { free(raw); return -1; }
    ...
    if (!zlib_data) { free(deflate_data); return -1; }
